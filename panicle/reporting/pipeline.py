...
def write_trait_results(
    ...
    output_prefix_base = str(context.output_dir / f"GWAS_{trait_name}")
    ...
    for method in ordered_methods:
        ...
        if isinstance(Res, FarmCPUResamplingResults):
            # Handle Resampling
            res_file = context.output_dir / f"GWAS_{trait_name}_{method}_RMIP.csv"
            df = Res.to_dataframe()
        ...
        if isinstance(method_metadata, dict) and method_metadata:
            meta_file = context.output_dir / f"GWAS_{trait_name}_{method}_metadata.json"
    ...
    if 'all_marker_pvalues' in outputs:
        maf_source = geno_for_maf or context.genotype_matrix
        ...
        all_res_df.to_csv(context.output_dir / f"GWAS_{trait_name}_all_results.csv", index=False)
    ...
    if hits_by_method and 'significant_marker_pvalues' in outputs:
        ...
        if np.any(any_hits):
            ...
            if 'MAF' not in sig_df.columns:
                sig_indices = np.where(any_hits)[0]
                geno_source = geno_for_maf or context.genotype_matrix
            ...
            sig_df.to_csv(context.output_dir / f"GWAS_{trait_name}_significant.csv", index=False)
