"""
Memory-Optimized GLM Implementation for Large-Scale GWAS

Key optimizations for 100k+ markers:
1. Streaming processing - no full data copies
2. Cache-optimized batch sizes based on CPU cache
3. In-place operations to minimize memory allocation
4. SIMD-optimized linear algebra with proper memory alignment
5. Adaptive processing strategy based on dataset size
6. Memory-mapped file support for ultra-large datasets

Target: 1-2 seconds for 100k markers with minimal memory usage
"""
...
def MVP_GLM_memory_optimized(phe: np.ndarray,
                           geno: Union[GenotypeMatrix, np.ndarray],
                           CV: Optional[np.ndarray] = None,
                           maxLine: int = 5000,
                           cpu: int = 1,
                           verbose: bool = True) -> AssociationResults:
    """Memory-optimized GLM for large-scale GWAS
    
    Optimizations:
    1. Streaming processing - no full data copies  
    2. Cache-optimized batch sizes
    3. Adaptive processing based on dataset size and memory
    4. In-place operations to minimize allocations
    5. SIMD-optimized linear algebra
    
    Args:
        phe: Phenotype matrix (n_individuals × 2)
        geno: Genotype matrix (n_individuals × n_markers) 
        CV: Covariate matrix (optional)
        maxLine: Hint for batch size (auto-optimized)
        cpu: CPU cores (currently ignored for memory optimization)
        verbose: Print progress
    
    Returns:
        AssociationResults with optimized memory usage
    """
    ...
    for start_idx, end_idx, chunk in chunk_iterator:
        ...
        if np.sum(valid_mask) > 0:
            ...
            chunk_pvals[valid_mask] = 2.0 * (1.0 - stdtr(df, np.abs(valid_t)))
        ...
        if verbose and end_idx - start_idx >= 10000:
...
MVP_GLM_ultra_optimized = MVP_GLM_memory_optimized