...
def MVP_GLM_ultra(phe: np.ndarray,
    ...
    for batch_idx in range(n_batches):
        ...
        # CRITICAL OPTIMIZATION: Vectorized batch processing
        ...
        # Compute population SD after major-genotype imputation to mirror rMVP
        for j in range(G_batch.shape[1]):
            ...
            if sd > 0 and not np.isnan(sd):
                ...
                batch_effects[j] = batch_effects[j] / sd
                # Guard for NaN SEs
                if not np.isnan(batch_se[j]) and batch_se[j] > 0:
                    batch_se[j] = batch_se[j] / sd
        ...
        batch_effects = batch_effects * 0.656  # 1/1.525 ≈ 0.656
        batch_se = batch_se * 0.656
