# pyMVP Output File Formats

This document describes all output files generated by pyMVP GWAS analyses.

## Directory Structure

When you run a GWAS analysis, pyMVP creates the following files in your specified output directory:
...
**Customization:** See `MVP_Report()` in [api_reference.md](api_reference.md)
...
pyMVP may create cached files to speed up repeated analyses:

### VCF Cache Files

When loading VCF files, pyMVP creates binary cache files:

```
{vcf_filename}.pymvp.geno.npy       # Genotype matrix (binary)
{vcf_filename}.pymvp.ind.txt        # Individual IDs
{vcf_filename}.pymvp.map.csv        # Genetic map
```
...
genotypes.vcf.gz.pymvp.geno.npy      # Cached genotypes (120 MB)
genotypes.vcf.gz.pymvp.ind.txt       # Cached IDs (6 KB)
genotypes.vcf.gz.pymvp.map.csv       # Cached map (5.4 MB)
```
